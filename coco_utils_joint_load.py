...
from util.DetectionLoader import Detection
...
def _coco_remove_images_without_annotations(dataset, cat_list=None):
    ...
    def _has_valid_annotation(anno):
        ...
        # for a in anno:
        #     if a["image_id"] not in [445972, 290868, 458898, 36942, 102540, 489235, 389232, 56878, 160679, 54374, 580945, 287310, 444171, 83023, 206539, 481370]:
        #         return False
        if len(anno) == 0:
            return False
...
def get_coco(root, image_set, transforms, mode='instances', toBinary=False, subset="all"):
    """
    ...
    if image_set == "train":
        PATHS = {
            "train_gt": ("train2017", os.path.join("/checkpoint/tarun05/data/COCO/COCO_from_SS", anno_file_template.format("instances", "train"))),
            "train_spp": ("train2017", os.path.join("/checkpoint/tarun05/data/COCO/COCO_from_SS", anno_file_template.format("part", "train"))),
            # "train": ("val2017", os.path.join("annotations", anno_file_template.format(mode, "val")))
        }
    else:
        PATHS = {
                    "train_gt": ("val2017", os.path.join("/checkpoint/tarun05/data/COCO/COCO_from_SS", anno_file_template.format("instances", "val"))),
                    "train_spp": ("val2017", os.path.join("/checkpoint/tarun05/data/COCO/COCO_from_SS", anno_file_template.format("part", "val"))),
                    # "train": ("val2017", os.path.join("annotations", anno_file_template.format(mode, "val")))
                }
...
if __name__ == "__main__":
    ...
    dataset_train = get_coco("/datasets01/COCO/022719", "train", presets.DetectionPresetTrain("hflip"), subset="coco")
    data_loader_train = DataLoader(dataset_train, batch_size=8, shuffle=False, num_workers=0, collate_fn=collate_fn)
