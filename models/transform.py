...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def postprocess_partial(self,
        ...
        for i, (pred, im_s) in enumerate(zip(result, image_sizes)):
            ...
            if "masks" in pred:
                masks = pred["masks"]
                masks = paste_masks_in_image(masks, boxes, im_s)
...
def resize_boxes(boxes, original_size, new_size):
    # type: (Tensor, List[int], List[int]) -> Tensor
    # original_size : resized image size after transform
    # new_size : original image size
    ratios = [
...
def resize_masks(masks, new_size):
    ...
    # new size : original image size

    h, w = new_size
    new_masks = F.interpolate(masks, size=(h, w), mode='bilinear', align_corners=False)
    return new_masks
